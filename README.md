...
observation error to get the mean resampled probability, $\bar{p_{ij}^*}$, as

$$\bar{p_{ij}^*} = p_{ij}(1+p_{fp})(1-p_{fn})$$

which yields the normal approximation

$$\mathcal{N}\bigg(n_p \bar{p_{ij}^*}, \sqrt{n_p \bar{p_{ij}^*} (1-\bar{p_{ij}^*})}\bigg)$$

which then can be converted back to a distribution of frequency of successes to
yield the final approximation

$$\mathcal{N}\bigg(\bar{p^*_{ij}},
\sqrt{\frac{\bar{p_{ij}^*}(1-\bar{p_{ij}^*})}{n_p}} \bigg)$$ 
{#eq:eq1}
